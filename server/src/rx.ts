...
function pyre(pattern:string): RegExp {
  ...
  pattern = replaceCaptureGroups(pattern, function (group: string): string {
...
function pyreReplacement(replacement:string, namedCaptures:string[]) {
	var jsReplacement = "";
	var i = 0;
	...
	while(i < replacementLength) {
		...
		if(cur == '\\' && i != (replacementLength - 1)) {
			var next: string = replacement[i + 1];
			var inext: integer = -1;
			if(next == '\\') {
				jsReplacement += '\\';
				i += 2;
				...
				if(replacement[i + 2] == "<" && closeIndex) {
					var group: string = replacement.substring(i + 3, closeIndex);
					var igroup: integer = -1;
					//if(isNaN(group)) {
 ...
 * @private
 */
function replaceCaptureGroups (pattern:string, fn: (group:string) =>string) {
	var start: integer = 0;
	var depth: integer = 0;
	var escaped = false;
