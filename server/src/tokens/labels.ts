...
function hasGuardingIfThatTerminates(doc: TextDocument, scopeStart: number, tokenStart: number, name: string): boolean {
	...
	const tokenLine = doc.positionAt(tokenStart).line;
	const startLine = doc.positionAt(scopeStart).line;

	for (let i = lines.length - 1; i >= 0; i--) {
		const line = lines[i];

		const inlineGuardRx = /^[ \t]*(?:yield\s+fail|jump\b|return\b|raise\b)[^\n]*?\bif[ \t]+(.+?)\s*$/;
		const inlineGuardMatch = inlineGuardRx.exec(line);
		if (inlineGuardMatch) {
			const condition = inlineGuardMatch[1];
		...
		const guardIsNone = /\bis\s+None\b/.test(condition);
		const guardIsNotNone = /\bis\s+not\s+None\b/.test(condition);
		...
		let hasIndentedBodyBeforeToken = false;
		let sawExitInThenBlock = false;
		for (let j = i + 1; j < lines.length; j++) {
			const l = lines[j];
			if (l.trim().length === 0) continue;
			const leading = l.match(/^[ \t]*/);
			const leadLen = leading ? leading[0].length : 0;
			if (leadLen <= ifIndent) break;
			const absLineNumber = startLine + j;
			if (guardIsNotNone) {
				hasIndentedBodyBeforeToken = true;
				break;
			}
			if (guardIsNone && termRx.test(l.trim())) {
				sawExitInThenBlock = true;
				break;
			}
			if (absLineNumber >= tokenLine) {
				break;
			}
		}

		if (guardIsNotNone && hasIndentedBodyBeforeToken) return true;
		if (guardIsNone && sawExitInThenBlock && tokenLine > startLine + i) return true;
	}
	return false;
}